...
const KpiTrackingPage: React.FC = () => {
  ...
  const handleSubmitKpi = useCallback(async () => {
    ...
    await fetchGeminiFeedback(prompt, 'Eres un coach de negocios experto en KPIs, evaluando la propuesta de un estudiante.');
  }, [currentScenario, kpiTitle, kpiDefinition, fetchGeminiFeedback, resetGemini]);
              ...
              <div>
                ...
                <div className="relative w-full">
                    ...
                    <Button
                        ...
                        className="absolute top-1/2 right-2 -translate-y-1/2 !p-2 h-8 w-8"
                        aria-label={recordingField === 'title' ? 'Detener grabación' : 'Grabar nombre por voz'}
                        disabled={!recognitionRef.current}
                    >
                        {recordingField === 'title' ? <StopCircleIcon className="w-4 h-4 text-red-500" /> : <MicrophoneIcon className="w-4 h-4" />}
                    </Button>
                </div>
              </div>
              <div>
                ...
                <div className="relative w-full">
                    ...
                    />
                     <Button
                        ...
                        disabled={!recognitionRef.current}
                    >
                        {recordingField === 'definition' ? <StopCircleIcon className="w-4 h-4 text-red-500" /> : <MicrophoneIcon className="w-4 h-4" />}
                    </Button>
                </div>
              </div>
            </div>
